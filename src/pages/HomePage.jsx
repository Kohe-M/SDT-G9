...
function calcPhaseInfo(now = new Date()) {
  ...
  return { phase, currentPeriod, nextPeriod, dayIdx, todayStr, minsUntilNext };
}
...
export default function HomePage() {
  ...
  const nextCardData = nextEntry
    ? { ...buildCardData(nextEntry, nextPeriod), openIn: minsUntilNext !== null ? Math.max(0, minsUntilNext - 30) : 0 }
    : null;

  const activeCls = phase === "pre"
    ? (nextEntry ? getClassByCode(nextEntry.code) : null)
    : (currentEntry ? getClassByCode(currentEntry.code) : null);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {currentData
        ...
        : <NoClassCard />}
      {nextData && <NextRow data={nextData} />}
    </div>
  );
}
...
function NoClassCard() {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 15, fontWeight: 700, color: C.ink }}>現在授業中の授業はありません</div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 11 }}>
        ...
        <span style={{ fontSize: 12, color: C.inkFaint, marginLeft: "auto", fontWeight: 500, whiteSpace: "nowrap" }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden" }}>
          <span style={{ fontSize: 14.5, fontWeight: 700, color: C.inkSoft, lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis" }}>
            {data.title}
          </span>
          <span style={{ flexShrink: 0, display: "inline-flex", alignItems: "center", gap: 3, fontSize: 12, fontWeight: 500, color: C.inkFaint }}>
...
function BocchiButton({ tone, label, sub, count, onPress }) {
  ...
  return (
    ...
    >
      <div style={{ width: 54, height: 54, borderRadius: 16, background: soft, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 9 }}>
          <span style={{ display: "flex" }}>
            {[0, 1, 2].map((i) => (
              <span key={i} style={{ width: 17, height: 17, borderRadius: 999, background: soft, border: `1.5px solid ${C.card}`, marginLeft: i ? -6 : 0, display: "inline-block" }} />
            ))}
          </span>
          <span style={{ fontSize: 11.5, fontWeight: 600, color: deep, whiteSpace: "nowrap" }}>いま{count}人が待ってる</span>
        </div>
      </div>
      <div style={{ width: 30, height: 30, borderRadius: 999, background: accent, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      >
        ...
        {searching ? (
          ...
          <div style={{ animation: "om-pop .4s ease both" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 18, background: C.card, border: `1px solid ${C.line}`, borderRadius: 20, padding: 16 }}>
