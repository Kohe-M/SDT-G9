...
import { C, BODY, Icon, Mascot } from "../components/DesignSystem";
...
export default function ChatPage() {
  ...
  const send = () => {
    ...
    // TODO(D担当): sendMessage(groupId, userId, text)
    setMessages(m => [...m, { id: Date.now(), me: true, text, time }]);
    setDraft("");
  };

  const title  = "ミクロ経済学Ⅰ のグループ";
  const members = 3;
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: C.bg }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: C.inkFaint, marginTop: 1, fontWeight: 500 }}>
            {members}人のメンバー
          </div>
        </div>
        <div style={{
      ...
      <div ref={scrollRef} className="noscroll" style={{ flex: 1, overflowY: "auto", padding: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 14 }}>
          {messages.map(m => m.me
            ...
            : <TheirBubble key={m.id} name={m.name} text={m.text} time={m.time} tone="sage" />
          )}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "flex-end", maxWidth: "82%" }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 11, color: C.inkFaint, fontWeight: 600, marginBottom: 4, paddingLeft: 2 }}>
          {name}
        </div>
